...
def run_rig_on_single_project(satdd, project_name):
    ...
    ensemble_voting(test_data, training_data)

    for test_dataset_name in all_datasets:
        if test_dataset_name in project_name:
            continue
        training_ids = np.where(np.array(training_data.data_pd['projectname']) == test_dataset_name)[0]

        training_data = satdd.create_and_process_dataset([test_dataset_name], doInclude=True)
        # no need to give a tfidf, will calculate itself
        training_data.set_csr_mat(max_f=MAX_FEATURE, stop_w=STOP_WORDS)
        test_data = satdd.create_and_process_dataset([project_name], doInclude=True)
...
def make_results(satdd):
    compare_with_huang("results/12_21_18_3/cross_proj_avg_f.txt", "results/12_21_18_cross_fea05_stopNone_f.csv")
    #process_output(satdd, '12_21_18_cross_fea05_stopNone.log', "12_21_18_3/")
